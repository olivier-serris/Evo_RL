...
# Adapt salina example to handle
# autoreset transitions and
...
def get_transitions(workspace):
    ...
    Filters every transitions [step_final,step_intial]
    ...
    for key in workspace.keys():
        array = workspace[key]
        # removes tranistions (s_terminal -> s_initial)
        x = array[:-1][~done]
        x_next = array[1:][~done]
...
def run_a2c(cfg):
    ...
    for epoch in range(cfg.algorithm.max_epochs):
        ...
        #!!! Convert workspace to transitions where (step_terminal => step initial) are removed
        replay_workspace = get_transitions(replay_workspace)
        #!!! Recreate truncated information
        truncated = (
            replay_workspace["env/timestep"] == cfg.algorithm.env.max_episode_steps
        )
        debug_checks(replay_workspace, truncated)

        critic, done, action_probs, reward, action = replay_workspace[
            "critic", "env/done", "action_probs", "env/reward", "action"
        ]

        #!!! Adapt loss to new transition tensor structure :
        ...
        entropy_loss = torch.distributions.Categorical(action_probs[0]).entropy().mean()
